...
class BSDS_RCFLoader(data.Dataset):
    ...
    def __init__(
        self, args, root='data/HED-BSDS_PASCAL', split='train', transform=False
    ):
        self.root = root
        self.split = split
        ...
        if self.split == 'train':
            self.filelist = join(
                self.root, 'train_{}_all_{}.lst'.format(args.type, args.fold)
            )

        elif self.split == 'test':
            self.filelist = join(
                self.root, 'test_{}_all_{}.lst'.format(args.type, args.fold)
            )
        else:
    ...
    def crop_500(self, img, gt_list):
        ...
        for index in range(len(gt_list)):
            ...
            gt_list[index] = gt_list[
                index
            ].float()  # if crop 500 + multi-scale to train, the 1280x720 is able to train
