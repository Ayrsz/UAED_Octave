...
from data.data_loader_one_random_drx_voc import BSDS_RCFLoader
...
test_dataset = BSDS_RCFLoader(
    root='/data/private/zhoucaixia/Dataset/data', split='test'
)
test_loader = DataLoader(
    test_dataset, batch_size=1, num_workers=1, drop_last=True, shuffle=False
)
...
for idx, image in enumerate(test_loader):
    ...
    io.savemat(
        join(mat_save_dir, '%s.mat' % filename), {'result': result}, do_compression=True
    )
