...
parser.add_argument(
    '--stepsize', default=3, type=int, metavar='SS', help='learning rate step size'
)
parser.add_argument(
    '--maxepoch',
    default=20,
...
parser.add_argument('--tmp', help='tmp folder', default='./temp/train')
parser.add_argument(
    '--dataset', help='root folder of dataset', default='./datasets/BSDS/'
)
...
TMP_DIR = join(
    THIS_DIR,
    args.tmp
    + '{}_{}_weightedstd{}_declr_adaexp'.format(
        MODEL_NAME[7:], args.distribution, args.std_weight
    ),
)
...
if os.path.exists(src_model):
    ...
    print(f'Aviso: Arquivo de modelo não encontrado em {src_model}')

copyfile(abspath(__file__), join(TMP_DIR, file_name))
        ...
        if i % args.print_freq == 0:
            info = (
                'Epoch: [{0}/{1}][{2}/{3}] '.format(
                    epoch, args.maxepoch, i, len(train_loader)
                )
            ...
            torchvision.utils.save_image(
                1 - mean, join(save_dir, 'iter-%d_mean.jpg' % i)
            )
...
def test(model, test_loader, epoch, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        io.savemat(
            join(mat_save_dir, '%s.mat' % filename),
            {'result': result},
            do_compression=True,
        )
