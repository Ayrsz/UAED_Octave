...
parser.add_argument(
    '--tmp',
    help='tmp folder',
    default='/data/private/zhoucaixia/workspace/UD_Edge/tmp/voc_pretrain_',
)
parser.add_argument(
    '--dataset',
    help='root folder of dataset',
    default='/data/private/zhoucaixia/Dataset/HED-BSDS/PASCAL',
)
...
TMP_DIR = join(
    THIS_DIR,
    args.tmp
    + '{}_{}_weightedstd{}_labelstd'.format(
...
copyfile(
    join(train_root, MODEL_NAME[:6], MODEL_NAME[7:] + '.py'),
    join(TMP_DIR, MODEL_NAME[7:] + '.py'),
)
...
def cross_entropy_loss_RCF(prediction, labelef):
    ...
    cost = F.binary_cross_entropy(prediction, labelef, weight=mask, reduction='sum')
